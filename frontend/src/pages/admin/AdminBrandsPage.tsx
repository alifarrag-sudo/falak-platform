 ...
 * Lists all brands with search, pagination.
 * Supports create, edit, and delete.
 */
import { useState } from 'react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { Search, Plus, Pencil, Trash2, ChevronLeft, ChevronRight, X, ShieldAlert, Briefcase } from 'lucide-react';
...
function CreateBrandModal({ onClose }: CreateModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
      <div className="bg-[#1c1c1c] border border-[#2a2a2a] rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2a2a2a]">
          <h2 className="text-base font-semibold text-white">Create Brand</h2>
...
function EditBrandModal({ brand, onClose }: EditModalProps) {
  const qc = useQueryClient();
  const [form, setForm] = useState({
    name: brand.name,
  ...
  const mutation = useMutation({
    mutationFn: (updates: Parameters<typeof updateBrand>[1]) => updateBrand(brand.id, updates),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin-brands'] });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
      <div className="bg-[#1c1c1c] border border-[#2a2a2a] rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2a2a2a]">
          <div>
            <h2 className="text-base font-semibold text-white">Edit Brand</h2>
...
function DeleteConfirmModal({ brand, onClose }: DeleteModalProps) {
  const qc = useQueryClient();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
      <div className="bg-[#1c1c1c] border border-[#2a2a2a] rounded-2xl w-full max-w-sm shadow-2xl p-6">
        ...
        <p className="text-sm text-gray-400 mb-6">
          You are about to permanently delete <span className="text-white font-medium">{brand.name}</span>.
        </p>
...
export default function AdminBrandsPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['admin-brands', search, page],
    queryFn: () => getBrands({
  ...
  return (
    <div className="space-y-5 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {total > 0 ? `${total} brand${total !== 1 ? 's' : ''} total` : 'Manage all platform brands'}
          </p>
        </div>
        <button
      ...
      <div className="card overflow-hidden">
        {isLoading ? (
          <div className="flex items-center justify-center h-48 text-gray-500 text-sm">Loading brands…</div>
        ) : isError ? (
          <div className="flex items-center justify-center h-48 text-red-400 text-sm">Failed to load brands. Check permissions.</div>
        ) : brands.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#2a2a2a]">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide hidden md:table-cell">Contact Email</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide hidden lg:table-cell">Budget Range</th>
                        ...
                        <div className="min-w-0">
                          ...
                          {brand.website && (
                            <a
                              ...
                              className="text-gray-500 text-xs truncate hover:text-green-400 transition-colors"
                            >
        ...
        {!isLoading && !isError && pages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-[#2a2a2a]">
            <p className="text-xs text-gray-500">
              Page {page} of {pages} &mdash; {total} brands
            </p>
      ...
      {editBrand    && <EditBrandModal brand={editBrand} onClose={() => setEditBrand(null)} />}
      {deleteTarget && <DeleteConfirmModal brand={deleteTarget} onClose={() => setDeleteTarget(null)} />}
    </div>
  );
}
