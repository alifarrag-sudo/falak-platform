...
router.get('/:id', async (req, res) => {
  ...
  `, [offer.id]) as Record<string, unknown>[];
...
router.post('/bulk', async (req, res) => {
  ...
  for (const influencer_id of influencer_ids) {
    try {
      ...
      const offer = await db.get('SELECT * FROM portal_offers WHERE id = ?', [id]) as Record<string, unknown>;
      created.push(offer);

      // Notify influencer (in-app + email)
      try {
        const inf = await db.get('SELECT name_english, email FROM influencers WHERE id = ?', [influencer_id]) as { name_english?: string; email?: string } | undefined;
        const campaign = campaign_id
          ? await db.get('SELECT name FROM campaigns WHERE id = ?', [campaign_id]) as { name?: string } | undefined
          : undefined;

        if (portalUser) {
          const pu = await db.get('SELECT email FROM portal_users WHERE id = ?', [portalUser.id]) as { email: string } | undefined;
          if (pu?.email) {
            const unifiedUser = await db.get('SELECT id FROM users WHERE email = ?', [pu.email]) as { id: string } | undefined;
            if (unifiedUser) {
              await createNotification(unifiedUser.id, 'offer_received', 'New offer received',
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (portalUser) {
      const portalUserFull = await db.get('SELECT email FROM portal_users WHERE id = ?', [portalUser.id]) as { email: string } | undefined;
      if (portalUserFull?.email) {
        const unifiedUser = await db.get('SELECT id FROM users WHERE email = ?', [portalUserFull.email]) as { id: string } | undefined;
        if (unifiedUser) {
  ...
  liveEmitter.emit('event', {
    type: 'new_offer',
    data: { id: offer.id, title: offer.title, influencer_id: offer.influencer_id, rate: offer.rate, currency: offer.currency },
...
router.put('/:id', async (req, res) => {
  ...
  const prevOffer = await db.get('SELECT * FROM portal_offers WHERE id = ?', [req.params.id]) as Record<string, unknown> | undefined;

  updates.updated_at = new Date().toISOString();
  const set = Object.keys(updates).map(k => `${k} = ?`).join(', ');
  const vals = Object.values(updates);
  await db.run(`UPDATE portal_offers SET ${set} WHERE id = ?`, [...vals, req.params.id]);

  const offer = await db.get('SELECT * FROM portal_offers WHERE id = ?', [req.params.id]) as Record<string, unknown>;
  ...
  if (prevOffer && newStatus && prevOffer.status !== newStatus && ['accepted', 'declined', 'completed'].includes(newStatus)) {
    try {
      ...
      if (prevOffer.campaign_id) {
        const campaign = await db.get('SELECT created_by FROM campaigns WHERE id = ?', [prevOffer.campaign_id]) as { created_by: string } | undefined;
      ...
      if (agencyUserId) {
        ...
        const agencyUser = await db.get('SELECT email, display_name FROM users WHERE id = ?', [agencyUserId]) as { email: string; display_name: string } | undefined;
  ...
  // Broadcast status change to live SSE clients
  if (prevOffer && updates.status && prevOffer.status !== updates.status) {
    liveEmitter.emit('event', {
      type: 'offer_update',
      data: { id: offer.id, title: offer.title, status: offer.status, influencer_id: offer.influencer_id },
      ts: new Date().toISOString(),
    });
  }
  ...
  try {
    if (counter_by === 'influencer') {
      ...
      if (offer.campaign_id) {
        ...
        if (campaign?.created_by) {
          const u = await db.get('SELECT id FROM users WHERE (display_name = ? OR email = ?) LIMIT 1', [campaign.created_by, campaign.created_by]) as { id: string } | undefined;
          agencyUserId = u?.id;
        }
      }
      if (!agencyUserId) {
      ...
      if (agencyUserId) {
        await createNotification(agencyUserId, 'offer_received',
      ...
      const pu = offer.portal_user_id
        ? await db.get('SELECT email FROM portal_users WHERE id = ?', [offer.portal_user_id]) as { email: string } | undefined
        : undefined;
      if (pu?.email) {
        const unifiedUser = await db.get('SELECT id FROM users WHERE email = ?', [pu.email]) as { id: string } | undefined;
        if (unifiedUser) {
          await createNotification(unifiedUser.id, 'offer_received',
...
router.post('/:id/accept-counter', async (req, res) => {
  const offer = await db.get('SELECT * FROM portal_offers WHERE id = ?', [req.params.id]) as Record<string, unknown> | undefined;
  if (!offer) return res.status(404).json({ error: 'Not found' });
...
/* ── Review deliverable ───────────────────────────────────── */
router.put('/:offerId/deliverables/:deliverableId/review', async (req, res) => {
  ...
  await db.run(`
    ...
    SET status = ?, feedback = ?, live_url = ?, reviewed_at = NOW()
    WHERE id = ? AND offer_id = ?
  `, [decision, feedback || null, live_url || null, req.params.deliverableId, req.params.offerId]);

  // Fetch offer details before updating for notification context
  const reviewedOffer = await db.get('SELECT * FROM portal_offers WHERE id = ?', [req.params.offerId]) as Record<string, unknown> | undefined;

  // If approved, auto-complete the offer and record live_at
  if (decision === 'approved') {
    await db.run(`
      UPDATE portal_offers SET status = 'completed', updated_at = NOW() WHERE id = ?
    `, [req.params.offerId]);
    if (live_url) {
      await db.run(`UPDATE portal_deliverables SET live_at = NOW(), live_url = ? WHERE id = ?`, [live_url, req.params.deliverableId]);
    }
    ...
    try {
      if (reviewedOffer?.portal_user_id) {
        ...
        `, [uuidv4(), reviewedOffer.portal_user_id, req.params.offerId]);
      }
      // Mark commission as collected
      await db.run(`UPDATE commissions SET status = 'COLLECTED', collected_at = NOW() WHERE reference_id = ?`, [req.params.offerId]);
    } catch { /* non-critical */ }

    // Notify the influencer (portal user) that their deliverable was approved
    try {
      if (reviewedOffer?.portal_user_id) {
        const pu = await db.get('SELECT email, name FROM portal_users WHERE id = ?', [reviewedOffer.portal_user_id]) as { email: string; name: string } | undefined;
        if (pu?.email) {
          // Email notification
    ...
    await db.run(`UPDATE portal_offers SET status = 'in_progress', updated_at = NOW() WHERE id = ?`, [req.params.offerId]);
  ...
  const deliverable = await db.get('SELECT * FROM portal_deliverables WHERE id = ?', [req.params.deliverableId]);
...
router.put('/:id/status', async (req, res) => {
  ...
  const set = Object.keys(updates).map(k => `${k} = ?`).join(', ');
  await db.run(`UPDATE portal_offers SET ${set} WHERE id = ?`, [...Object.values(updates), req.params.id]);

  const offer = await db.get('SELECT * FROM portal_offers WHERE id = ?', [req.params.id]);
  res.json(offer);
});
...
router.post('/:id/send', async (req, res) => {
  ...
  await db.run(`UPDATE portal_offers SET status = 'sent', sent_at = NOW(), updated_at = NOW() WHERE id = ?`, [req.params.id]);

  // Notify portal user if linked
  try {
    if (offer.portal_user_id) {
      const pu = await db.get('SELECT email FROM portal_users WHERE id = ?', [offer.portal_user_id]) as { email: string } | undefined;
      if (pu?.email) {
        sendOfferReceivedEmail(pu.email, {
